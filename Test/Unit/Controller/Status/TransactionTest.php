...
use PayPro\Przelewy24\Controller\Status\Transaction;

class TransactionTest extends StatusControllerTestCase
{
    /**
     * @var \PayPro\Przelewy24\Controller\Status\Transaction
     */
    private $controller;
    ...
    {
        $this->requestMock->expects($this->once())->method('getContent')->willReturn('{"data": "transaction_data"}');
        $this->webhookHandlerMock->expects($this->once())->method('handle')->with(['data' => 'transaction_data']);
    ...
    public function testError(): void
    {
        ...
        $this->webhookHandlerMock->expects($this->once())
            ->method('handle')
            ->with(['data' => 'transaction_data'])
            ->willThrowException(new \Exception('Transaction webhook error'));
